...
def ui_execution_select(current_digital_twin) -> None:
    try:
        ...
        execution_options = {
            str(
                execution["_id"]
            ): f"{execution.get('title')}: {len(execution['workflowSchema']['components'])} steps"
        ...
        with ui.column():
            try:
                name_input = ui.input(
                    ...
                    placeholder="name",
                    validation={
                        "Can not be empty": lambda value: len(value.strip()) != 0
                    },
                ).props("size=300")
                components = storage.get_active_object_from_storage("components")
...
def ui_workarea(current_digital_twin):
    ...
    if not current_digital_twin:
        ...
        ui.label(
            f"{current_user.get('display_name')} / {current_digital_twin.get('name')}"
        )
        components = storage.get_active_object_from_storage("components")
        if storage.app_storage_is_set("components"):
            ...
            ui.button(
                "Change components", on_click=lambda: ui.open(ui_theme.PATH_COMPONENTS)
            )
        else:
            ui.button(
                "Select components", on_click=lambda: ui.open(ui_theme.PATH_COMPONENTS)
            )
...
def add_execution(digital_twin_id, name, components):
    try:
        if digital_twin_id and name and components:
            ...
            ui.notify(
                f"A execution with id {execution_id} has been created", type="positive"
            )
    except Exception as e:
        ui.notify(
            f"Execution could not be added. An exception occured:", type="negative"
        )
    finally:
