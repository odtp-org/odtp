...
def ui_users_select() -> None:
    ...
    try:
        ...
        ui.notify(f"User selection could not be loaded. An Exception occured: {e}", type="negative")  
...
def ui_workarea():
    ...
    try:
        ...
        if user:
            ...
            ui.button("Manage Digital Twins", 
                      on_click=lambda: ui.open(ui_theme.PATH_DIGITAL_TWINS))  
    except Exception as e:
        ui.notify(f"Workarea could not be retrieved. An Exception occured: {e}", type="negative")             
...
def store_selected_user(value):
    try:
        ...
        ui.notify(f"Selected user could not be stored. An Exception occured: {e}", type="negative")
    finally:
        ui_workarea.refresh()
...
def add_user(name, github, email):
    if (name and github and email):
        try:
            ...
            ui.notify(f"The user could not be added in the database. An Exception occured: {e}", 
                      type="negative")
        finally:
