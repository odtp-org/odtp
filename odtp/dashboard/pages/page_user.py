...
def ui_users_select() -> None:
    ...
    try:
        ...
        logging.error(f"User selection could not be loaded. An Exception occurred: {e}")
...
def store_selected_user(value):
    ...
    try:
        ...
        logging.error(f"Selected user could not be stored. An Exception happened: {e}")
    else:
...
async def pick_workdir() -> None:
    try:
        ...
        logging.error(f"Work directory could not be picked: an Exception occurred: {e}")
    else:
        ui_workarea.refresh()
