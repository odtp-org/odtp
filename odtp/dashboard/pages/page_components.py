...
def ui_components_select() -> None:
    ...
    try:
        ...
        components_options = {
            str(component["_id"]): f"{component.get('componentName')}: ({component.get('repoLink')})" 
...
def ui_version_select() -> None:
    try:
        ...
        if current_component:
            ...
            if versions:
                version_options = {
                    str(version["_id"]): f"component-version: {version['component_version']} odtp-version: {version['odtp_version']} commit: {version.get('commitHash')}"
        ...
        ui.notify(f"Version selection could not be loaded. An Exception occured: {e}", 
                  type="negative")
...
def ui_component_register():
    with ui.column().classes('w-full'):
        ...
        component_name_input = ui.input(
            label='Component name', placeholder='component name',
            validation={'File not valid': lambda value: lambda value: len(value.strip()) >= 4}).classes('w-2/3')
        repo_link_input = ui.input(
            label='Repository URL', placeholder='repo url',
            validation={'repo url not valid': lambda value: lambda value: len(value.strip()) >= 4}).classes('w-2/3')
        ui.markdown("""
                    #### Version
                    """)
        component_version_input = ui.input(
            label='Component version', placeholder='component version',
            validation={'version number not valid': lambda value: lambda value: len(value.strip()) >= 4}).classes('w-1/3')
        odtp_version_input = ui.input(
            label='ODTP version', placeholder='ODTP version',
            validation={'version number not valid': lambda value: lambda value: len(value.strip()) >= 4}).classes('w-1/3')
        commit_hash_input = ui.input(
            label='Commit hash', placeholder='commit',
            validation={'commit not valid': lambda value: lambda value: len(value.strip()) >= 4}).classes('w-2/3')
        ui.button(
            'Register component',
...
def store_selected_component(value):
    try:
        ...
        ui.notify(f"Selected component could not be stored. An Exception occured: {e}", 
                  type="negative")
    finally:     
        ui_workarea.refresh()
...
def add_selected_component_version(component_id, version_id, replace):
    try: 
        storage.storage_update_version(component_id=component_id,
                                    version_id=version_id,
                                    replace=replace)
    except Exception as e:
        ui.notify(f"Selected component version could not be stored. An Exception occured: {e}", 
                  type="negative")    
    finally:
        ui_workarea.refresh()
        ui_version_select.refresh()
...
def register_component(
    ...
    try:
        ...
        ui.notify(f"The component and version could not be added. An Exception occured: {e}", 
                  type="negative")        
    finally:  
