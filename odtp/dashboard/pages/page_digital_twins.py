...
def ui_digital_twins_table(current_user):
    try:
        ...
        if digital_twins:
            ...
            df["executions"] = df["executions"].apply(format.pd_lists_to_counts).astype("string")
            df = df[["_id", "name", "status", "created_at", "updated_at", "executions"]]
            ui.table.from_pandas(df)
        else:
            ui.label("You don't have digital twins yet. Start adding one.")
    except Exception as e:
        ui.notify(f"Digital Twin table could not be loaded. An Exception occured: {e}", 
                  type="negative")              
...
def ui_digital_twin_select(current_user) -> None:
    try:
        ...
        ui.notify(f"Digital Twin Selection could not be loaded. An Exception occured: {e}", 
                  type="negative")          
...
def ui_add_digital_twin(current_user):
    ...
    with ui.column().classes('w-full'):
        name_input = ui.input(label='Name', placeholder='name',
                                validation={'Can not be empty': lambda value: len(value.strip()) != 0}).classes('w-2/3')

        ui.button('Add new digital twin',
                    on_click=lambda: add_digital_twin(name=name_input.value,
                                                    user_id=current_user["user_id"]))  
...
def store_selected_digital_twin_id(value):
    try:
        ...
        ui.notify(f"Selected digital twin could not be stored. An Exception occured: {e}", 
                  type="negative")
    finally:        
        ui_workarea.refresh()
...
def add_digital_twin(name, user_id):
    if name and user_id:
        try:
            ...
            ui.notify(f"A digital_twin with id {digital_twin_id} has been created", type="positive")
        except Exception as e:
            ui.notify(f"The digital twin could not be added in the database. An Exception occured: {e}", 
                      type="negative")            
        finally:        
            ui_digital_twin_select.refresh()
        ...
        ui.notify(f"A digital twin can only be added once the form is filled", type="negative")
