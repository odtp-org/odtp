...
class ExecutionTable:
    ...
    def add_header(self):
        headers = [
            ...
            {"text": "Title", "col_span": 1},
            {"text": "Id", "col_span": 2},
            {"text": "Created At", "col_span": 1},
            {"text": "Updated At", "col_span": 1},
            {"text": "Ended at", "col_span": 1},
        ]
        with ui.row().classes("w-full bg-gray-200 p-2 border-b grid grid-cols-10 gap-4 flex items-center justify-items-start"):
    ...
    def add_rows(self):
        ...
        for execution in self.filtered_executions:
            if execution.get("deprecated"):
                ...
                color = ""
            with ui.row().classes("w-full p-2 border grid grid-cols-10 gap-4 flex items-center justify-items-start"):
                ui.checkbox(
                    on_change=lambda e, execution_id=execution["_id"]: self.toggle_selection(e.value, execution_id)
                )
                ui.label(execution['title']).classes(f"text-center truncate {color} col-span-1")
                ui.label(str(execution['_id'])).classes(f"text-center truncate {color} col-span-2")
                ui.label(execution['createdAt'].strftime(f'%Y-%m-%d')).classes("text-center truncate {color} col-span-1")
                if execution.get('start_timestamp'):
                    ui.label(execution['start_timestamp'].strftime(f'%Y-%m-%d')).classes("text-center truncate {color} col-span-1")
                else:
                    ui.label("not run").classes(f"{color}")
                if execution.get('end_timestamp'):
                    ui.label(execution['end_timestamp'].strftime('%Y-%m-%d')).classes(f"text-center truncate {color} col-span-1")
                else:
                    ui.label("not ended").classes(f"{color} col-span-1")
