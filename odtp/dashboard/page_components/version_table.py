...
class VersionTable:
        ...
        for version in self.filtered_versions:
            with ui.row().classes("w-full p-2 border grid grid-cols-10 gap-4 items-center"):
                checkbox = ui.checkbox(
                    on_change=lambda e, version=version: self.toggle_selection(e.value, version["version_id"])
                )
                ...
                ui.label(version.get('deprecated', "False")).classes("truncate")
    ...
    def rebuild_rows(self):
        print("in rebuild")
        print(type(self.show_deprecated))
        print(self.show_deprecated)
        ...
        print(len(self.filtered_versions))
        self.add_rows.refresh()

    def delete_selected(self):
        print(self.selected_version_ids)
